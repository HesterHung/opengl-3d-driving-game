...
tunnelObj = JSONNurbsLoader(os.path.join(os.path.dirname(__file__), "nurbs/nurbs_export.json"), color=(0.4, 0.4, 0.4))
...
def isColliding(checkX=None, checkZ=None):
    # Use provided coordinates or default to Jeep's current position
    x = checkX if checkX is not None else jeepObj.posX
    z = checkZ if checkZ is not None else jeepObj.posZ

    current_hitbox_radius = ckSense * jeepObj.sizeX 

    for obstacle in obstacleCoord: 
        d = math.sqrt((x - obstacle[0])**2 + (z - obstacle[1])**2)
        if d <= current_hitbox_radius: 
            return True

    # Check Streetlights
    # Use a tighter radius for poles so we don't get blocked too early
    pole_hitbox = 2.0 
    for sl in allstreetlights:
        d = math.sqrt((x - sl.posX)**2 + (z - sl.posZ)**2)
        if d <= pole_hitbox:
            return True
            
    safe_road_width = land - (2.0 * jeepObj.sizeX)
    
    if (x >= safe_road_width or x <= -safe_road_width): 
        return True
...
def display():
    global jeepObj, canStart, score, beginTime, countTime, lightMode, timeLeft, GAME_DURATION, helpWindow
        ...
        # --- 2. BOSS BATTLE LOGIC ---
            ...
            for p in projectiles[:]:
                ...
                if dist((jeepObj.posX, jeepObj.posZ), (p.posX, p.posZ)) < hit_radius: 
                    ...
                    next_posZ = jeepObj.posZ + p.velZ * seconds_passed
                    
                    # 2. Check if this push would cause a collision (Wall or Obstacle)
                    if isColliding(next_posX, next_posZ):
                         # The player is being pinned against something.
                         # Destroy the projectile to free them.
                         projectiles.remove(p)
                         continue 

                    # 3. Apply velocity to player (Push them)
                    jeepObj.posX = next_posX
                    jeepObj.posZ = next_posZ
...
def drawHelpOverlay():
    ...
    # Draw Semi-transparent background (Smaller Box)
    glEnable(GL_BLEND)
    ...
    # Draw Text (Adjusted coordinates)
    glColor3f(1.0, 0.0, 0.0)
    drawTextBitmap("Help Guide" , -0.15, 0.65)
    
    glColor3f(0.0, 1.0, 0.0)
    drawTextBitmap("Jeep Controls:", -0.7, 0.55) 
    glColor3f(1.0, 1.0, 1.0)
    drawTextBitmap("Up/Down Arrows: Move Forward / Backward", -0.65, 0.47)
    drawTextBitmap("Left/Right Arrows: Turn Jeep Left / Right", -0.65, 0.39)
    drawTextBitmap("Spacebar: Stop wheel rotation (Brake)", -0.65, 0.31)
    drawTextBitmap("'+' / '-': Increase / Decrease Jeep Size", -0.65, 0.23)
    drawTextBitmap("'l': Toggle Headlights", -0.65, 0.15) 

    glColor3f(0.0, 1.0, 0.0)
    drawTextBitmap("Camera Controls:", -0.7, 0.0) 
    glColor3f(1.0, 1.0, 1.0)
    drawTextBitmap("Middle Mouse + Drag: Orbit Camera", -0.65, -0.08)
    drawTextBitmap("'z' / 'x': Zoom In / Zoom Out", -0.65, -0.16)
    drawTextBitmap("'t': Toggle Top-Down View", -0.65, -0.24)
    drawTextBitmap("'b': Toggle Behind-Jeep View", -0.65, -0.32)
    drawTextBitmap("'c': Reset Camera to Default", -0.65, -0.40) 

    glColor3f(0.0, 1.0, 0.0)
    drawTextBitmap("Other:", -0.7, -0.55)
    glColor3f(1.0, 1.0, 1.0)
    drawTextBitmap("'h': Close this Help Window", -0.65, -0.63)
    drawTextBitmap("Right Mouse Click: Open Main Menu", -0.65, -0.71) 
